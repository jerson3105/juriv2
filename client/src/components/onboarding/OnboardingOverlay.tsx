...
export const OnboardingOverlay = () => {
  ...
  // Calcular posición del spotlight y tooltip
  const calculatePositions = useCallback(() => {
    ...
    const tooltipHeight = 300; // Altura real del tooltip con todos los elementos
    let tooltipTop = 0;
    let tooltipLeft = 0;
    ...
    // Si el tooltip queda muy abajo, moverlo arriba del elemento
    if (tooltipTop + tooltipHeight > window.innerHeight - 16) {
      tooltipTop = rect.top - tooltipHeight - 20;
      // Si aún no cabe arriba, centrarlo verticalmente
      if (tooltipTop < 16) {
        tooltipTop = Math.max(16, (window.innerHeight - tooltipHeight) / 2);
      }
    }
  ...
  useEffect(() => {
    if (isActive) {
      ...
      // Recalcular después de un pequeño delay para elementos que se renderizan tarde
  ...
  // Mostrar modal centrado si no hay selector o no se encontró el elemento
    ...
    <AnimatePresence>
      ...
      >
        ...
        {spotlightPosition && (
          ...
          >
            <div className="w-full h-full rounded-xl border-2 border-violet-400 shadow-[0_0_20px_rgba(139,92,246,0.5)]" />
          </motion.div>
        )}
        ...
        >
          ...
          <div className="p-5">
            ...
            <div className="flex items-center justify-between gap-3">
              {step?.requiresAction ? (
                // Si requiere acción, mostrar solo botón de omitir
                <>
                  <div /> {/* Spacer */}
                  <button
                    onClick={skipOnboarding}
                ...
                // Si no requiere acción, mostrar navegación normal
                <>
                  <button
                    onClick={prevStep}
                  ...
                  <button
                    ...
                    className="flex items-center gap-1 px-4 py-2 bg-gradient-to-r from-violet-500 to-purple-600 text-white text-sm font-medium rounded-lg shadow-lg shadow-violet-500/25 hover:shadow-xl transition-all"
                  >
                    {isLastStep() ? (
                      <>
                        <Sparkles size={16} />
